...
def get_multilingual_header(header_text, default="ascii"):
    if not header_text is None:
        ...
        try:
            header_sections = [unicode(text, charset if charset and charset != 'unknown' else default, errors='replace')
...
def AnalyseMessage(message, processesMultipartMessages=[]):
    ...
    if not InIgnore(message):
        ...
        with open(os.path.join(currentFolder['output'], currentFolder['root'] + "." + currentFolder['name'] + ".dat"),
                  "a") as myfile:
