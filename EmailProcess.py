...
class gmail(abstract_email):
    ...
    def __init__(self, con, email=None):
        """init with connextion data and email object inicialiced"""
        self.conx = con
        self.mail=email
        if email==None:
        ...
        for folder in self.folder_list:
            ...
            try:
              ...
              print("Reading (" + str(name_folder) + "):"+str(size_folder)+" email inside")
                ...
                print("error in reading ("+str(name_folder)+") for prepare the dict of size and data :"+str(read_exp))
    ...
    def jsonifyMessage(self, msg):
        ...
        try:
           ...
           print("error parsing Date :"+json_msg['Date']+" Error:"+str(e))
        ...
        try:
            ...
            try:
              self.mail.select(folder)
                ...
                logging.warning("folder not read" + str(read_exp))
                logging.warning("error in mail.select("+folder+")")
                return msg_list
            result, data = self.mail.uid('search', None, "ALL")
            # search and return uids instead
                    ...
                    try:
                        if isinstance(response_part, tuple):
                            ...
                            try:
                                ...
                                logging.warning("error in response part" + str(read_exp))
                            response_part_string = response_part[1].decode('utf-8',errors='ignore')
                            id_message_string=response_part[0].decode('utf-8');
                            msg = email.message_from_string(response_part_string)
                            msg_list.append(msg)
                            logging.debug("number of messages:"+str(id_message_string))
                    except Exception as read_exp:
                        logging.error(str(read_exp))
