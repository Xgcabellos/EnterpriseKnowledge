...
from employee_email_worker import emailworker, message_html, read_knowlege_info, read_users_info
...
def active_log(log_name='process.log', level=logging.INFO, log_directory='./logs/'):
    ...
    logging.info('Starting mail_process logger using v.' + str(
...
for know in knowledge:
    ...
    for book in listofbooks:
        ...
        module_logger.debug('Finish book: ' + title + '. Messages processed: ' + str(
            len(list_ofmessage_proceesd)) + ' messages of the ' + str(
            number_of_messages_prepared) + ' prepared')

    try:
        module_logger.debug('Finish. send ' + str(len(list_ofmessage_proceesd)) + ' messages of the ' + str(
            number_of_messages_prepared) + ' prepared')
