...
def processFolders(reader, name_folder, _FROM_EMAIL, output_queue):
    ...
    if JSON_STORE_TYPE == 'file':
        json_store = JsonStorageEmail(log_name, log_level_json, log_directory)
...
def main():
    ...
    graph_store = neo4J_storageEmail(driver, log_name, log_level_graph, log_directory)
    ...
    if type_of_email == 'PST':
        pst = pst_process.PstFile('../input/xavier.pst', './output', 'report.info', log_directory='output')
        pst.read_all()
    ...
    for index, user in usersinfo.iterrows():  # Reading all users
        ...
        if type_of_email == 'IMAP_SSL':
            ...
            properties = google_connexionProperties(_ORG_EMAIL, FROM_EMAIL, FROM_PWD)
            ...
            try:
                ...
                properties = email_IMAP_connexionProperties(_ORG_EMAIL, FROM_EMAIL, FROM_PWD, False, EMAIL_SERVER, 143)
